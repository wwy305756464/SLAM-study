...
int main(int argc, char **argv) {
	...
	cv::Mat image = cv::imread(image_file, 0); // 这里读取进来的是灰度图，CV_8UC1
	int rows = image.rows, cols = image.cols;
	cv::Mat image_undistorted = cv::Mat(rows, cols, CV_8UC1); // 去畸变之后的图

	// 不用openCV内置函数，但是自己写的去畸变函数
	for (int v = 0; v < rows; v++) {
		for (int u = 0; u < cols; u++) {
			...
			if (u_distorted >= 0 && v_distorted >= 0 && u_distorted < cols && v_distorted < rows){
				...
				// image.at<uchar>(y, u) -> get a pixel's value from an image, at (v, u)
			}
			else {
